...
//supports passing in a function to extract a specific property of each array item that will be pushed onto result array
module.exports.buildArrayFromPagingParams = function(fullArray, pagingParams, extractItemFunction){
  var arrayToReturn = [];
  var itemCounter = 0;
  for(var arrayItem of fullArray){
    ...
    if(extractItemFunction===undefined){
      arrayToReturn.push(arrayItem);
    }
    else{
      arrayToReturn.push(extractItemFunction(arrayItem));
    }
