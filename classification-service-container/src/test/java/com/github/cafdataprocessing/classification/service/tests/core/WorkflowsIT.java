...
/**
 * Integration tests for Workflow API paths in the Classification Service.
 */
public class WorkflowsIT {
    ...
    public void deleteWorkflow() throws ApiException {
        ...
        BaseClassificationRule classificationRule = ObjectsInitializer.initializeClassificationRule(null);
        classificationRulesApi.createClassificationRule(projectId, remainingWorkflow.getId(), classificationRule);

        try{
            workflowsApi.deleteWorkflow(projectId, remainingWorkflow.getId());
            Assert.fail("Exception should have been thrown when trying to delete a workflow with a classification rule under it.");
        }
        catch(ApiException e){
            Assert.assertTrue(e.getMessage().contains("Unable to Delete. There are Classification Rules on the Workflow."),
    ...
    public void updateWorkflow() throws ApiException {
        ...
        //verify that trying to update a workflow that doesn't exist fails
        try{
            workflowsApi.updateWorkflow(projectId, ThreadLocalRandom.current().nextLong(), updatedWorkflow);
            Assert.fail("Expected an exception to be thrown trying to update a workflow that doesn't exist.");
        }
        catch(ApiException e){
            Assert.assertTrue(e.getMessage().contains("Unable to find Workflow with ID"),
                    "Expected message to state workflow could not be found. Message: "+e.getMessage());
        }
    }
